// TODO: Flip index schema so keywords point to objects
...
function parseAndWriteData(d) {
    ...
    for (const [key, value] of Object.entries(parsedData)) {
        parsedData[key].keywords = keywordize(value.name);
    }

    // departments
    for (const course of Object.values(d.courses)) {
        if (Object.keys(parsedData).includes(course.department)) continue;
        parsedData[course.department] = {
            type: 'DEPARTMENT',
            name: course.department_name,
            keywords: [...course.department_alias, ...keywordize(course.department_name)],
        };
    }

    // courses
    for (const [id, data] of Object.entries(d.courses)) {
        parsedData[id] = {
            type: 'COURSE',
            department: data.department,
            number: data.number,
            keywords: keywordizeName(data.title),
        };
    }

    // instructors
    for (const instructor of Object.values(d.instructors)) {
        parsedData[instructor.shortened_name] = {
            type: 'PROFESSOR',
            name: instructor.name,
            keywords: keywordizeName(instructor.name),
        };
    }
